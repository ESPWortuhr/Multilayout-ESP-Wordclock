#!/usr/bin/env python3
"""Render Uhrtype front matrix definitions from the C++ headers.

The simulator intentionally reads the existing `include/Uhrtypes/*.hpp` files
instead of duplicating the matrix definitions in Python. This makes it useful
for checking coordinate changes while editing an Uhrtype.
"""
...
UHRTYPES_DIR = REPO_ROOT / "include" / "Uhrtypes"
...
def parse_uhrtype(path: Path) -> Uhrtype | None:
    if path.name == "Uhrtype.hpp":
        return None

    text = path.read_text(encoding="utf-8")
    class_match = re.search(r"class\s+(\w+)_t\s*:\s*public\s+iUhrType", text)
    if not class_match:
        return None
...
def load_uhrtypes() -> list[Uhrtype]:
    return sorted(
        (u for path in UHRTYPES_DIR.glob("*.hpp") if (u := parse_uhrtype(path))),
        key=lambda uhrtype: uhrtype.name.lower(),
    )


def find_uhrtype(uhrtypes: list[Uhrtype], requested: str) -> Uhrtype:
    wanted = normalize_name(requested)
    exact = [u for u in uhrtypes if normalize_name(u.name) == wanted]
    if exact:
        return exact[0]
    ...
    if len(matches) == 1:
        return matches[0]
    if not matches:
        raise SystemExit(f"Unknown Uhrtype {requested!r}. Use --list to see names.")

    names = ", ".join(u.name for u in matches)
    raise SystemExit(f"Ambiguous Uhrtype {requested!r}: {names}")
...
def set_daytime_words(uhrtype: Uhrtype, hour: int, words: list[str]) -> None:
    if not uhrtype.has("hasDaytimeWords"):
        return
...
def has_twenty_and_check_for_usage(
    uhrtype: Uhrtype, *, use_twenty: bool, use_forty: bool
) -> bool:
    return uhrtype.has("hasTwenty") and (use_twenty or use_forty or uhrtype.lang == "ES")


def has_dreiviertel_and_check_for_usage(uhrtype: Uhrtype, *, use_45: bool) -> bool:
    if not uhrtype.has("hasDreiviertel"):
        return False
    return uhrtype.lang != "DE" or use_45
...
def show_minute(
    ...
    *,
    ...
    if not residual:
        return
    if uhrtype.has("hasSpecialWordBeen"):
        words.append("nur" if residual == 0 else "gewesen")
    if uhrtype.has("hasMinuteInWords") and minute_variant == "in-words":
...
def set_minute(
    ...
    *,
    ...
    if uhrtype.has("has60MinuteLayout"):
        words.append("uhr")
        if minute == 0:
            full_hour = True
        else:
    ...
    if uhrtype.has("hasOnlyQuarterLayout"):
        if 8 <= minute <= 22:
            if use_15:
    ...
    if minute == 0:
        ...
        if uhrtype.has("hasThirtyfive"):
            words.extend(["min_35", "nach"])
        elif uhrtype.has("hasTwentyfive"):
    ...
    if uhrtype.normalized_name == "RU10X11":
        offset_hour = 0
    return offset_hour, full_hour


def set_hour(uhrtype: Uhrtype, hour: int, full_hour: bool, words: list[str]) -> None:
    midnight = uhrtype.has("hasMitternacht") and hour % 24 == 0
...
def set_it_is(uhrtype: Uhrtype, hour: int, minute: int, words: list[str], variant: str) -> None:
    ...
    if uhrtype.normalized_name == "RU10X11":
        if hour == 1:
            ...
            words.append("es_ist")
    elif uhrtype.normalized_name == "ES10X11" and hour == 1 and rounded_minute(minute) < 35:
        words.append("es_ist___plural___")
    elif uhrtype.normalized_name == "ES10X11" and hour == 0 and rounded_minute(minute) > 34:
        words.append("es_ist___plural___")
    else:
        words.append("es_ist")
...
def words_for_time(
    ...
    *,
    ...
    set_hour(uhrtype, hour + offset_hour, full_hour, words)
    set_daytime_words(uhrtype, hour + offset_hour, words)
    set_it_is(uhrtype, hour + offset_hour, minute, words, it_is_variant)
    return words
...
def print_available_words(uhrtype: Uhrtype) -> None:
    for word in sorted(uhrtype.actions):
        print(word)


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Render an Uhrtype definition from include/Uhrtypes/*.hpp."
    )
    parser.add_argument("uhrtype", nargs="?", help="Uhrtype name, e.g. DE16x18")
    parser.add_argument("--list", action="store_true", help="List parsed Uhrtypes")
    ...
    parser.add_argument("--available-words", action="store_true", help="List words for an Uhrtype")
    ...
    uhrtypes = load_uhrtypes()

    if args.list:
        for uhrtype in uhrtypes:
            marker = "layout" if uhrtype.layout else "no-layout-comment"
            print(f"{uhrtype.name:28} {uhrtype.rows:2}x{uhrtype.cols:<2} {uhrtype.lang:2} {marker}")
        return 0

    if not args.uhrtype:
        parser.error("uhrtype is required unless --list is used")

    uhrtype = find_uhrtype(uhrtypes, args.uhrtype)
    if not uhrtype.layout:
        raise SystemExit(f"{uhrtype.name} has no parseable layout comment to render.")

    if args.available_words:
        print_available_words(uhrtype)
        return 0
    ...
    print(f"{uhrtype.name} ({uhrtype.rows}x{uhrtype.cols})")
    print("words:", " ".join(words))
    if missing:
        print("missing mappings:", " ".join(missing), file=sys.stderr)
    if args.active_letters:
        print("active:", uhrtype.active_letters(lit))
    print(
        uhrtype.render(
            lit,
