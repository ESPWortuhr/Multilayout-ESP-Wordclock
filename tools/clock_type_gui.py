#!/usr/bin/env python3
"""Local web GUI for the ClockType simulator."""
...
import clock_type_simulator as sim
...
<head>
  ...
  <title>ClockType Simulator</title>
  <style>
    :root {
      color-scheme: dark;
      --bg: #17191c;
...
<body>
  <div class="app">
    <aside>
      <div>
        <h1>ClockType Simulator</h1>
        <div class="subline" id="layoutMeta">-</div>
      </div>

      <div class="field">
        <label for="clock_type">ClockType</label>
        <select id="clock_type"></select>
      </div>
  ...
  <script>
    ...
    const controls = ["clock_type", "time", "timeRange", "use15", "use20", "use40", "use45", "itIs", "minuteVariant"];
    ...
    function selectedMeta() {
      return state.clock_types.find((item) => item.name === $("clock_type").value);
    }
    ...
    async function init() {
      const response = await fetch("/api/clock_types");
      state.clock_types = await response.json();
      const select = $("clock_type");
      state.clock_types.forEach((item) => {
      ...
      const preferred = state.clock_types.find((item) => item.name === "DE16x18");
      select.value = preferred ? preferred.name : state.clock_types.find((item) => item.has_layout).name;
      setTime("12:00");
...
def cells_for_render(clock_type: sim.ClockType, lit: set[tuple[int, int]]) -> list[list[dict[str, object]]]:
    rows: list[list[dict[str, object]]] = []
    for row_index in range(clock_type.rows):
        raw = clock_type.layout[row_index] if row_index < len(clock_type.layout) else ""
        raw = raw.ljust(clock_type.cols)[: clock_type.cols]
        row: list[dict[str, object]] = []
        for display_col, char in enumerate(raw):
            code_col = clock_type.cols - display_col - 1
...
class Handler(BaseHTTPRequestHandler):
    clock_types = sim.load_clock_types()
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/":
            self.send_html()
            return
        if parsed.path == "/api/clock_types":
            self.send_json(
                [
                    {
                        "name": clock_type.name,
                        "rows": clock_type.rows,
                        "cols": clock_type.cols,
                        "lang": clock_type.lang,
                        "has_layout": bool(clock_type.layout),
                        "supported_minute_variants": clock_type.supported_minute_variant_mask(),
                    }
    ...
    def render_api(self, params: dict[str, list[str]]) -> None:
        try:
            clock_type = sim.find_clock_type(self.clock_types, params.get("clock_type", ["DE16x18"])[0])
        ...
        self.send_json(
            {
                "name": clock_type.name,
                "rows_count": clock_type.rows,
                "cols_count": clock_type.cols,
                "lang": clock_type.lang,
                "words": words,
                "missing": missing,
                "active_letters": clock_type.active_letters(lit),
                "supported_minute_variants": clock_type.supported_minute_variant_mask(),
                "rows": cells_for_render(clock_type, lit),
            }
        )


def main() -> int:
    parser = argparse.ArgumentParser(description="Start the local ClockType simulator GUI.")
    ...
    print(f"ClockType simulator GUI: {url}")
    if args.open:
        webbrowser.open(url)
    try:
