...
<script>var MINI=require("minified");var _=MINI._,$=MINI.$,$$=MINI.$$,EE=MINI.EE,HTML=MINI.HTML;var ip_esp="ws://192.168.4.1";var debug=false;var command=1;var rgb=[[0,0,100],[0,10,0],[10,0,0],[5,5,5]];var sl_typ=0;var hell=2;var geschw=10;var anzahl=100;var sleep=0;var sleeptime=1;var position=100;var anzahl=100;var color=0;var h6=100;var h8=100;var h12=100;var h16=100;var h18=100;var h20=100;var h22=100;var h24=100;var zeige_sek=0;var zeige_min=0;var ldr=0;var ldrCal=0;function init_websocket(){var a=ip_esp;websocket=new WebSocket(a);websocket.onopen=function(b){$("#_websocket").fill("Online ");$("#_websocket").set("$backgroundColor","#4DED4D");$("#_websocket").set("$color","#000000");$("#_websocket").set("@value","1");send_data(301,0,0)};websocket.onclose=function(b){$("#_websocket").fill("Offline ");$("#_websocket").set("$backgroundColor","#ED4D4D");$("#_websocket").set("$color","#ffffff");$("#_websocket").set("@value","0")};websocket.onmessage=function(c){if(debug==true){$("#output").fill("RESPONSE: "+c.data)}var b=JSON.parse(c.data);if(b.command=="config"){$("#_ssid").set("value",b.ssid);$("#_mm").set("value",b.minute);$("#_ss").set("value",b.sekunde);$("#_zs").set("value",b.zeitserver);$("#_hn").set("value",b.hostname);$("#_lt").set("value",b.ltext);$("#_h6").set("value",b.h6);$("#_h8").set("value",b.h8);$("#_h12").set("value",b.h12);$("#_h16").set("value",b.h16);$("#_h18").set("value",b.h18);$("#_h20").set("value",b.h20);$("#_h22").set("value",b.h22);$("#_h24").set("value",b.h24);$("#_rg6").set("value",b.hell);$("#_rg7").set("value",b.geschw);$("#_zeige_sek").set("value",b.zeige_sek);$("#_zeige_min").set("value",b.zeige_min);$("#_ldr").set("value",b.ldr);$("#_ldrCal").set("value",b.ldrCal)}if(b.command=="set"){rgb[0][0]=b.rgb00;rgb[0][1]=b.rgb01;rgb[0][2]=b.rgb02;rgb[1][0]=b.rgb10;rgb[1][1]=b.rgb11;rgb[1][2]=b.rgb12;rgb[2][0]=b.rgb20;rgb[2][1]=b.rgb21;rgb[2][2]=b.rgb22;rgb[3][0]=b.rgb30;rgb[3][1]=b.rgb31;rgb[3][2]=b.rgb32;hell=b.hell;geschw=b.geschw;set_slider()}};websocket.onerror=function(b){if(debug==true){$("#output").fill("ERROR: "+b.data)}}}function clock(){var a=new Date();var c=a.getHours();var f=a.getMinutes();var b=a.getSeconds();var e=a.getTimezoneOffset();if(c<10){c="0"+c}if(f<10){f="0"+f}if(b<10){b="0"+b}var d=c+":"+f+":"+b;$("#_clock").fill(d);setTimeout("clock()",1000)}clock();function toArray(a){return Array.prototype.slice.call(a)}setTimeout(function(){window.scrollTo(0,1)},1);function wait(a){var c=new Date().getTime();for(var b=0;b<10000000;b++){if((new Date().getTime()-c)>a){break}}}function get_slider(){rgb[sl_typ][0]=$("#_rg1").get("value");rgb[sl_typ][1]=$("#_rg2").get("value");rgb[sl_typ][2]=$("#_rg3").get("value");hell=$("#_rg6").get("value");geschw=$("#_rg7").get("value");anzahl=$("#_rg9").get("value");position=$("#_rg91").get("value")}function set_slider(){$("#_rg1").set("value",rgb[sl_typ][0]);$("#_rg2").set("value",rgb[sl_typ][1]);$("#_rg3").set("value",rgb[sl_typ][2]);$("#_rg6").set("value",hell);$("#_rg7").set("value",geschw);$("#_rg9").set("value",anzahl);$("#_rg91").set("value",position);$("#rg1wert").fill(rgb[sl_typ][0]);$("#rg2wert").fill(rgb[sl_typ][1]);$("#rg3wert").fill(rgb[sl_typ][2])}function nstr(a){if(a<10){a="00"+a}else{if(a<100){a="0"+a}}return a}function send_data(d,c,b){var a=nstr(d)+nstr(c)+nstr(b)+nstr(rgb[0][0])+nstr(rgb[0][1])+nstr(rgb[0][2])+nstr(rgb[1][0])+nstr(rgb[1][1])+nstr(rgb[1][2])+nstr(rgb[2][0])+nstr(rgb[2][1])+nstr(rgb[2][2])+nstr(rgb[3][0])+nstr(rgb[3][1])+nstr(rgb[3][2])+nstr(hell)+nstr(geschw)+nstr(anzahl)+nstr(position)+"999";websocket.send(a);if(debug==true){$("#output").fill(a)}}$.ready(function(){var a=location.host;if(a!=0){ip_esp="ws://"+a}set_slider();init_websocket();$("#_websocket").on("click",function(){var b=$(this).get("value");if(b=="1"){b=0;$("#_websocket").set("$backgroundColor","#EDED19");$("#_websocket").set("$color","#000000");$("#_websocket").fill("..working..");$("#_websocket").set("value",b);websocket.close()}else{b=1;$("#_websocket").set("$backgroundColor","#EDED19");$("#_websocket").set("$color","#000000");$("#_websocket").fill("..working..");$("#_websocket").set("value",b);init_websocket()}return false});$("#_clock").on("click",function(){var b=new Date();var e=b.getHours();var j=b.getMinutes();var d=b.getSeconds();var h=b.getTimezoneOffset();var g=b.getTime()/1000;h=h/60*-1;var f="030000000"+nstr(h)+g;for(var c=0;c<30;c++){if(f.length<30){f+=" "}}f+="999";if(debug==true){$("#output").fill(f)}websocket.send(f)});$("#_speichern").on("click",function(){send_data(20,0,0);return false});$("#_fb").on("click",function(){sl_typ=$(this).get("value");if(sl_typ=="2"){sl_typ=0;$("#_fb").set("$backgroundColor","#e9e9e9");$("#_fb").fill("Vordergrund");$("#_fb").set("value",sl_typ)}else{if(sl_typ=="0"){sl_typ=1;$("#_fb").set("$backgroundColor","#4B97D6");$("#_fb").fill("Hintergrund");$("#_fb").set("value",sl_typ)}else{sl_typ=2;$("#_fb").set("$backgroundColor","#cd5c5c");$("#_fb").fill("Rahmenfarbe");$("#_fb").set("value",sl_typ)}}set_slider();return false});$(('[id*="_pbutton"]')).on("click",function(){var c=$(this).get("id");$(('[id*="_pbutton_"]')).set("$backgroundColor","#e9e9e9");var b=new RegExp("^_pbutton_");if(b.test(c)){$("#"+c).set("$backgroundColor","#4B97D6")}$(('[id*="cl_"]')).set({$display:"none"});if(c=="_pbutton0"){$(".cl_start").set({$display:"block"});$(".cl_rgb1").set({$display:"block"});$(".cl_rgb2").set({$display:"block"});sl_typ=0;$("#_fb").set("$backgroundColor","#e9e9e9");$("#_fb").fill("Vordergrund");$("#_fb").set("value",sl_typ);command=1;send_data(command,0,0);set_slider()}if(c=="_pbutton1"){$(".cl_effekte").set({$display:"block"});command=1}if(c=="_pbutton2"){$(".cl_setup").set({$display:"block"});command=999;send_data(300,0,0)}if(c=="_pbutton_3"){$(".cl_effekte").set({$display:"block"});$(".cl_rgb2").set({$display:"block"});sl_typ=3;command=204;send_data(command,0,0);set_slider()}if(c=="_pbutton_4"){$(".cl_effekte").set({$display:"block"});$(".cl_rgb2").set({$display:"block"});sl_typ=3;command=200;send_data(command,0,0);set_slider()}if(c=="_pbutton_5"){$(".cl_effekte").set({$display:"block"});$(".cl_rgb2").set({$display:"block"});$(".cl_geschw").set({$display:"block"});sl_typ=3;command=201;send_data(command,0,0);set_slider()}if(c=="_pbutton_6"){$(".cl_effekte").set({$display:"block"});$(".cl_hell").set({$display:"block"});$(".cl_geschw").set({$display:"block"});command=202;send_data(command,0,0);set_slider()}if(c=="_pbutton_7"){$(".cl_effekte").set({$display:"block"});$(".cl_hell").set({$display:"block"});$(".cl_geschw").set({$display:"block"});command=203;send_data(command,0,0);set_slider()}});$('[id*="_fbutton"]').on("click",function(b){var c=$(this).get("id");if(c=="_fbutton1"){rgb[sl_typ][0]=255,rgb[sl_typ][1]=0;rgb[sl_typ][2]=0}if(c=="_fbutton2"){rgb[sl_typ][0]=0,rgb[sl_typ][1]=255;rgb[sl_typ][2]=0}if(c=="_fbutton3"){rgb[sl_typ][0]=0,rgb[sl_typ][1]=0;rgb[sl_typ][2]=255}if(c=="_fbutton4"){rgb[sl_typ][0]=255,rgb[sl_typ][1]=255;rgb[sl_typ][2]=0}if(c=="_fbutton5"){rgb[sl_typ][0]=0,rgb[sl_typ][1]=255;rgb[sl_typ][2]=255}if(c=="_fbutton6"){rgb[sl_typ][0]=90,rgb[sl_typ][1]=90;rgb[sl_typ][2]=90}if(c=="_fbutton7"){rgb[sl_typ][0]=0,rgb[sl_typ][1]=0;rgb[sl_typ][2]=0}hell=$("#_rg6").get("value");geschw=$("#_rg7").get("value");anzahl=$("#_rg9").get("value");position=$("#_rg91").get("value");set_slider();send_data(command,0,0);set_slider();return false});$('[id*="_rg"]').onChange(function(c){var d=$(this).get("id");if(sleep==0){get_slider();if(d=="_rg1"){send_data(command,1,0)}if(d=="_rg2"){send_data(command,1,0)}if(d=="_rg3"){send_data(command,1,0)}if(d=="_rg6"){send_data(251,0,0)}if(d=="_rg7"){send_data(252,0,0)}if(d=="_rg9"){send_data(253,0,0)}if(d=="_rg91"){send_data(254,0,0)}set_slider();sleep=1;sleeptime=20;var b=setTimeout(function(){sleep=0},sleeptime)}return false});$("#_wlan").on("click",function(){var e=$(_ssid).get("value");var b=$(_passwd).get("value");var d="099000000"+e;for(var c=0;c<25;c++){if(d.length<34){d+=" "}}d+=b;for(var c=0;c<25;c++){if(d.length<59){d+=" "}}d+="999";websocket.send(d);$("#output").fill("WLAN wurde neu konfiguriert");if(debug==true){$("#output").fill(d)}return false});$("#_zeitserver").on("click",function(){var d=$(_zs).get("value");var c="097000000"+d;for(var b=0;b<25;b++){if(c.length<25){c+=" "}}c+="999";websocket.send(c);$("#output").fill("Zeitserver wurde neu konfiguriert");if(debug==true){$("#output").fill(c)}return false});$("#_ltext").on("click",function(){var b=$(_lt).get("value");var d="096000000"+b;for(var c=0;c<39;c++){if(d.length<39){d+=" "}}d+="999";websocket.send(d);$("#output").fill("Lauftext wurde neu konfiguriert");if(debug==true){$("#output").fill(d)}});$("#_helligkeit").on("click",function(){h6=$(_h6).get("value");h8=$(_h8).get("value");h12=$(_h12).get("value");h16=$(_h16).get("value");h18=$(_h18).get("value");h20=$(_h20).get("value");h22=$(_h22).get("value");h24=$(_h24).get("value");var b="095000000"+nstr(h6)+nstr(h8)+nstr(h12)+nstr(h16)+nstr(h18)+nstr(h20)+nstr(h22)+nstr(h24)+"999";websocket.send(b);$("#output").fill("Helligkeit wurde neu konfiguriert");if(debug==true){$("#output").fill(b)}});$("#_minute").on("click",function(){zeige_min=$(_zeige_min).get("value");var b="094000000"+zeige_min+"  999";websocket.send(b);$("#output").fill("Anzeige Minute wurde neu konfiguriert");if(debug==true){$("#output").fill(b)}return false});$("#_ldrsave").on("click",function(){ldr=$(_ldr).get("value");ldrCal=$(_ldrCal).get("value");var b="091000000"+ldr+"  "+ldrCal+"  999";websocket.send(b);$("#output").fill("LDR Einstellung wurde neu konfiguriert");if(debug==true){$("#output").fill(b)}return false});$("#_sekunde").on("click",function(){zeige_sek=$(_zeige_sek).get("value");var b="093000000"+zeige_sek+"  999";websocket.send(b);$("#output").fill("Anzeige Sekunde wurde neu konfiguriert");if(debug==true){$("#output").fill(b)}return false});$("#_host").on("click",function(){var c=$(_hn).get("value");var d="092000000"+c;for(var b=0;b<25;b++){if(d.length<25){d+=" "}}d+="999";websocket.send(d);$("#output").fill("Hostname wurde neu konfiguriert");if(debug==true){$("#output").fill(d)}return false});$("#_reset").on("click",function(){send_data(100,0,0)})});</script>
