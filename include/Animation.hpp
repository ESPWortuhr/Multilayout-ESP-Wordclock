...
Rain rain[ANIMATION_COLUMNS] = { Rain(), };
// ###############################################################################
class Ball {
public:
	Ball() {};
	...
	void begin(int row, int column, RgbaColor foreground, RgbaColor background, int delay) {
		this->delay = delay;
	...
	int move(int timedelta) {
		if ( ! end) {
			...
			if (delay <= 0) {
				int _vy = vy;
				y += (((g * timedelta) / 1000) * timedelta) / 2000 + (vy * timedelta) / 1000;
				vy += (g * timedelta) / 1000;
				...
				lastDown |= (_vy < 0) && (vy >= 0) && (y > lastPos);	// oberer Wendepunkt
				r = y >> 8;
			}
...
// ###############################################################################
	...
	Animation::Ani Animation::isSilvester(struct tm &tm) {
		...
		if (lastMinute != _minute) {
			lastMinute = _minute;
			if ((tm.tm_mon == 12) && (tm.tm_mday == 31) && (tm.tm_hour == 23) && (_minute == 59)) {
//			if ((month(ltime) == 12) && (day(ltime) == 12) && (hour(ltime) == 14) && (_minute == 35)) {
//			if (_minute == G.autoLdrBright) {					// for testing
				minutesActive = 0;
	...
	void Animation::changeBrightness() {
	    ...
	    analyzeColors(NULL, STRIPE, newForeground, newBackground);		// nur Vorder- Hintergrund aus LED stripe ermitteln
	    bool adjustFg = newForeground != foreground, adjustBg = newBackground != background;
...
//		Serial.println("Save Matrix");
		...
		analyzeColors(neu, STRIPE, foreground, background);				// copy from stripe to neu[][]
		if (isColorization()) {
	...
	//------------------------------------------------------------------------------
	// kopiere (interne Matrix oder vom LED Stripe) und ermittle Vorder- und Hintergrundfarbe
	void Animation::analyzeColors(RgbaColor (*dest)[ANIMATION_COLUMNS], RgbaColor (*source)[ANIMATION_COLUMNS], RgbaColor &foreground, RgbaColor &background) {
		RgbaColor color, color1(0), color2(0);
		int colorCounter1 = 0, colorCounter2 = 0;
		for (int r = 0; r < ANIMATION_ROWS; r++) {
			for (int c = 0; c < ANIMATION_COLUMNS; c++) {
				if (source == STRIPE) {
					color = led_get_pixel(getIndex[r + ANIMATION_ROW_START][c + ANIMATION_COLUMNS_START]);
				} else {
					color = source[r][c];
				}
				if (dest != NULL) {
	...
	void Animation::copy2Stripe(RgbaColor (*source)[ANIMATION_COLUMNS]) {
		for (int r = 0; r < ANIMATION_ROWS; r++) {
			for (int c = 0; c < ANIMATION_COLUMNS; c++) {
				...
				led_set_pixel_Color_Object((uint16_t)getIndex[r + ANIMATION_ROW_START][c + ANIMATION_COLUMNS_START], source[r][c]);
			}
		}
	}
	//------------------------------------------------------------------------------
	...
	//------------------------------------------------------------------------------
	void Animation::fillMatrix(RgbaColor (*matrix)[ANIMATION_COLUMNS], RgbaColor color, bool fg) {
		for (int r = 0; r < ANIMATION_ROWS; r++) {
			for (int c = 0; c < ANIMATION_COLUMNS; c++) {
	...
	bool Animation::led_show_notify(bool changed, uint8_t minute) {
		...
		if (G.animType == KEINE) {
			if ((changed == 1) && (lastMinute != minute)) {
				lastMinute = minute;
				matrixChanged = true;		// fuer den Fall, dass die Animation eingeschaltet wird
			}
		} else  {
			switch (changed) {
			case 0:
		...
		if ((G.prog == COMMAND_IDLE) || (G.prog == COMMAND_MODE_WORD_CLOCK)) {
			...
			if (G.animType == KEINE) {
				...
				if (( ! isIdle()) && (now >= nextActionTime)) {
					...
					if ((G.animType != lastAnimType) || (G.animDuration != lastAnimSpeed) || (G.animColorize != lastAnimColor) || (G.animDemo != lastAnimDemo)) {
						lastAnimType = (Ani)G.animType;
...
//					Serial.printf("Anim: type %d phase %d\n", G.animType, phase);
	...
	void Animation::blockgrafik(uint8_t rot, uint8_t gruen, uint8_t blau, BLOCKS num) {
		for (int r = 0; r < ANIMATION_ROWS; r++) {
			for (int c = 0; c < ANIMATION_COLUMNS; c++) {
	...
	void Animation::copyBlock(RgbaColor color, uint block, bool fgbg, bool mirrored, bool init) {
		if ( ! init) {
...
uint Animation::calcDelay(uint frames) {
	...
	case 1:  pause = 2000 / frames; break;	// 2 seconds
	case 3:  pause = 5000 / frames; break;	// 5 seconds
	default: pause = 3500 / frames; break;	// 3.5 seconds
	}
	if (pause == 0) {
		pause = 1;
	}
		...
		for (c = 0; (c < ANIMATION_COLUMNS) && (numBalls < MAX_BAELLE); c++) {
			for (r = 0; (r < ANIMATION_ROWS) && (numBalls < MAX_BAELLE); r++) {
				if (work[r][c].Foreground) {
					baelle[numBalls].begin(r, c, work[r][c], background, 100 * numBalls);
					numBalls++;
					break;
				}
			}
	...
	for (int c = 0; c < ANIMATION_COLUMNS; c++) {
		for (int r = 0; r < ANIMATION_ROWS; r++) {
			if (sparkle) {
				...
				if (tmp[r][c].Foreground) {
					color = color.LinearBlend(background, tmp[r][c], progress);		// blend in
					work[r][c].changeRgb(color);
				} else {
					if (alt[r][c].Foreground) {
						color = color.LinearBlend(alt[r][c], background, progress);	// blend out
						work[r][c].changeRgb(color);
						tmp[r][c] = work[r][c];
					}
				}
...
void Animation::set_pixel_for_char(uint8_t i, uint8_t h, uint8_t offset, unsigned char unsigned_d1, HsbColor color) {
...
uint Animation::animCountdown(struct tm &tm) {
	...
	if (_second != lastSecond) {
		...
		char seconds[8];
		sprintf(seconds, "%d", countDown);		// start 23:59:00		60 - 0    for (uint8_t i = 0; i < 5; i++) {
	    for (uint8_t i = 0; i < 5; i++) {		// column
	        for (uint8_t h = 0; h < 8; h++) {	// row
	        	if (countDown >= 10) {
	        		// 1. Zahl ohne Offset
	        		set_pixel_for_char(i, h, 0, static_cast<unsigned char>(seconds[0]), hsbColor_1);
	        		// 2. Zahl mit Offset
	        		set_pixel_for_char(i, h, 6, static_cast<unsigned char>(seconds[1]), hsbColor_2);
	        	} else {
	        		set_pixel_for_char(i, h, 3, static_cast<unsigned char>(seconds[0]), hsbColor_1);
	        	}
	        }
        }
	    countDown--;
...
void Animation::animColorChange() {
	...
	static uint32_t pauseZeitColor = 50;	// 20 Wechsel / Sekunde -> 1200 W. / Minute
	...
	if (isColorization() && (G.animSpeed > 0)) {
		...
		if (now >= (lastTimeColor + pauseZeitColor)) {
			lastTimeColor = now;
                 				// Farbwinkel 0.0 - 1.0
			float deltaHue = fmod(1.0/(G.animSpeed * 20.0), 1.0);	// Alle Farben in 1 .. 60 Sekunden / 0 == aus
			HsbColor hsbColor;
			for (int r = 0; r < ANIMATION_ROWS; r++) {
				for (int c = 0; c < ANIMATION_COLUMNS; c++) {
					if (work[r][c].Foreground) {
...
uint32_t Animation::animMatrixRain() {
	...
	if (phase == 1) {
		animationDelay = calcDelay(frames);
		memcpy(alt, work, sizeof(alt));		// work is still the previous animated array
		int stop;
	...
	for (c = 0; c < ANIMATION_COLUMNS; c++) {
		for (r = 0; r < ANIMATION_ROWS; r++) {
			...
			rainColor = rainColor.LinearBlend(fadeColor, rainColor, rainColor.Alpha);
