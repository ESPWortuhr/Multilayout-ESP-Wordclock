...
protected:
	...
	void set_pixel_for_char(uint8_t i, uint8_t h, uint8_t offset, unsigned char unsigned_d1, HsbColor color);
	bool isIdle();
	...
	void saveMatrix();
	void analyzeColors(RgbaColor (*dest)[ANIMATION_COLUMNS], RgbaColor (*source)[ANIMATION_COLUMNS], RgbaColor &foreground, RgbaColor &background);
	void copy2Stripe(RgbaColor (*source)[ANIMATION_COLUMNS]);
	void copy2Work(RgbaColor (*source)[ANIMATION_COLUMNS]);
	void copyBlock(RgbaColor color, uint block, bool fgbg, bool mirrored, bool init = false);
	void fillMatrix(RgbaColor (*matrix)[ANIMATION_COLUMNS], RgbaColor color, bool fg);
